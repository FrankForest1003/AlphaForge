# AlphaForge Local LEAN Runtime v1.1.0 全套使用说明
...
```text
D:\Code\alphaforge-local-lean-runtime-v1.1.0
```
...
```text
~/Code/alphaforge-local-lean-runtime-v1.1.0
```

进入目录：

```bash
cd ~/Code/alphaforge-local-lean-runtime-v1.1.0
chmod +x scripts/*.sh
```
...
```powershell
Set-Location D:\Code\alphaforge-local-lean-runtime-v1.1.0
