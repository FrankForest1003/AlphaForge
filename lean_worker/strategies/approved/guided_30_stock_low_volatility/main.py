...
class GuidedThirtyStockLowVolatility(AlphaForgeBaseAlgorithm):
    """Monthly low-volatility ranking for the controlled Guided Mode."""
    ...
    def initialize_strategy(self):
        ...
        self.lookback = int(self._parameter("lookback", "63"))
        ...
        self.set_warm_up(
            max(self.lookback + 5, self.risk_sma_period + 5),
            Resolution.DAILY,
        )
    ...
    def rebalance(self):
        ...
        if self.risk_filter_enabled and (
            ...
            self.af_record_signal(
                "guided_low_volatility_risk_off",
                {
        ...
        minimum_observations = max(10, self.lookback // 2)
        for symbol in self.symbols:
            frame = frames.get(symbol.value)
            if frame is None or "close" not in frame.columns:
                continue
            returns = frame["close"].astype(float).pct_change().dropna().tail(self.lookback)
            if len(returns) < minimum_observations:
                continue
            volatility = float(returns.std() * (252.0 ** 0.5))
            if volatility > 0:
                annualized_volatility[symbol] = volatility

        ranked = sorted(annualized_volatility.items(), key=lambda item: item[1])
        selected = [symbol for symbol, _ in ranked[: self.top_k]]
        if len(selected) < self.top_k:
            self.af_record_signal(
                "guided_low_volatility_insufficient_history",
                {
        ...
        self.af_record_signal(
            "guided_low_volatility_rank",
            {
                "annualized_volatility": {
                    symbol.value: volatility
                    for symbol, volatility in ranked
                },
        ...
        self.af_rebalance_to_weights(
            ...
            "Monthly low-volatility rebalance",
        )

    def on_alpha_end(self):
        self.debug("ALPHAFORGE_GUIDED_30_LOW_VOLATILITY_COMPLETED")
